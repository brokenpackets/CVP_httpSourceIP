...
"""
This file is intended to be used as a CVP configlet builder. It creates an aliases file 
called /etc/profile.d/00-aliases.sh, which will automatically apply to all user profiles. 
The goal is to automatically map curl to curl --interface for all users, so that CVP image 
downloads can utilize the `ip http client local-interface` equivalent for CVP 2021+.
...
rcFileBody = '''\
#!/bin/bash echo alias curl=\\'curl --interface {interface}\\' > /etc/profile.d/00-aliases.sh
'''.format(interface=updateSource)
###
...
def uploadFile(fileBody,fileLocation,fileName,sudo=False):
    ...
    if sudo == True:
        uploadFile = ss.runCmds( 1, [ 'enable', 'bash timeout 2 echo "'+fileStripped+'" | base64 -d > /tmp/'+fileName])
        moveFile = ss.runCmds( 1, ['enable', 'bash timeout 2 sudo su -c \"mv /tmp/'+fileName+' '+fileLocation+fileName+'\" -s /bin/bash root'])
    else:
